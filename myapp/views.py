...
from myapp.forms import SignUpForm,LoginForm,ProfileEditForm,CoverPicForm
...
def remove_comment_view(request,*args,**kwargs):
    ...
    comment_obj=Comments.objects.get(id=id).delete()
...
class ProfileListView(ListView):
    ...
    #     qs=UserProfile.objects.exclude(request.user)
...
def follow_view(request,*args,**kwargs):
    id=kwargs.get("pk")
    profilew_obj=UserProfile.objects.get(id=id)
    user_prof=UserProfile.objects.get(user=request.user)
    user_prof.following.add(profilew_obj)
    user_prof.save()
...
def unfollow_view(request,*args,**kwargs):
    ...
    profilew_obj=UserProfile.objects.get(id=id)
    user_prof=UserProfile.objects.get(user=request.user)
    user_prof.following.remove(profilew_obj)
    user_prof.save()
    print(f"{request.user} is following  {user_prof.following.all()}")
